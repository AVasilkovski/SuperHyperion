...
class VisualEvidenceAgent(BaseAgent):
    ...
    def _check_vlm_availability(self):
        ...
        try:
            ...
            vision_models = [
                m for m in models if any(v in m for v in ["llava", "bakllava", "moondream"])
            ]
    ...
    async def extract_from_image(self, image_path: str, page_number: int = 1) -> ExtractedData:
        """
...
Respond with just the category name."""
    ...
    async def _extract_chart_data(
        self, image_path: str, page_number: int, media_type: str
    ) -> ExtractedData:
...
Be precise with numbers."""
    ...
    async def _extract_table_data(self, image_path: str, page_number: int) -> ExtractedData:
...
4. Key findings or trends if applicable"""
    ...
    async def _get_data_bbox(self, image_path: str, data: Optional[Dict]) -> Optional[BoundingBox]:
    ...
    async def verify_extraction(
        self, original_path: str, extracted_data: ExtractedData
    ) -> Tuple[bool, float]:
        """
