...
from src.agents.belief_agent import BeliefMaintenanceAgent, BeliefState
...
class TestBeliefMaintenanceAgent:
    ...
    def test_bayesian_update_supporting_evidence(self, agent):
        """Supporting evidence should increase alpha."""
        ...
        assert agent.db.query_delete.called
    
    def test_bayesian_update_refuting_evidence(self, agent):
        """Refuting evidence should increase beta."""
        agent.db.query_fetch = Mock(return_value=[{
            "beta_alpha": 5.0,
            "beta_beta": 3.0,
        }])
    ...
    def test_bayesian_update_weighted_evidence(self, agent):
        """Evidence weight should affect update magnitude."""
        agent.db.query_fetch = Mock(return_value=[{
            "beta_alpha": 5.0,
            "beta_beta": 5.0,
        }])
        ...
        # Stronger evidence should still work
        assert agent.db.query_delete.called
