...
def test_auth_missing_token_fails_closed_401(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        get_auth_context(MockRequest(), authorization=None, x_tenant_id="tenant-1", x_role="operator")
...
def test_insecure_header_fallback_only_when_env_enabled_and_dev(monkeypatch):
    ...
    auth_ctx = get_auth_context(MockRequest(), authorization=None, x_tenant_id="tenant-123", x_role="operator")
    assert auth_ctx.tenant_id == "tenant-123"
    ...
    monkeypatch.setattr(config.auth, "env", "prod")
    with pytest.raises(HTTPException) as exc:
        get_auth_context(MockRequest(), authorization=None, x_tenant_id="tenant-123", x_role="operator")
    assert exc.value.status_code == 401
