...
def get_migrations(migrations_dir: Path) -> list[tuple[int, Path]]:
    ...
    if valid_files:
        ...
        if first_ord == 1:
            ...
            if (
                "schema_version" not in content
                or "ordinal" not in content
                or "git-commit" not in content
                or "applied-at" not in content
            ):
                raise ValueError(
                    f"Migration 001 must contain 'schema_version' definitions with 'ordinal', 'git-commit', 'applied-at'. Found in {first_path.name}: missing standard keywords."
                )
...
def apply_migration(driver, db: str, migration_file: Path, next_ordinal: int, dry_run: bool):
    ...
    if not any(schema.lower().startswith(kw) for kw in ["define", "undefine", "redefine"]):
        raise ValueError(
            f"Migration hygiene violation: {migration_file.name} must start with define/undefine/redefine. Found: {schema[:20]}..."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to apply SCHEMA transaction for {migration_file.name} (Ordinal: {next_ordinal}): {e}"
        ) from e
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to apply WRITE transaction for {migration_file.name} (Ordinal: {next_ordinal}): {e}"
        ) from e
...
def main():
    ...
    p.add_argument(
        "--migrations-dir", default="schema/migrations", help="Directory containing NNN_*.tql files"
    )
    ...
    p.add_argument(
        "--dry-run", action="store_true", help="Print planned migration actions without executing."
    )
