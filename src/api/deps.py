...
def get_tenant_id(auth: AuthContextV1 = Depends(get_auth_context)) -> str:
    """Extract tenant identifier dynamically. Fail-closed if missing."""
    return auth.tenant_id


def get_role(auth: AuthContextV1 = Depends(get_auth_context)) -> RoleEnum:
...
def verify_operator_role(auth: AuthContextV1 = Depends(get_auth_context)) -> RoleEnum:
    ...
    if role not in (RoleEnum.OPERATOR, RoleEnum.ADMIN):
        raise HTTPException(
            status_code=403, detail="Operation requires operator or admin privileges"
        )
    return role


def get_request_context(auth: AuthContextV1 = Depends(get_auth_context)) -> Tuple[str, RoleEnum]:
